...
from market_maker_sim.market_data.generator import OrderFlowGenerator, GeneratorConfig
from market_maker_sim.market_maker.strategy import StrategyConfig, NaiveMarketMaker, ASMarketMaker
...
def run_sim(mm_class, steps=10000):
    ...
    mm_config = StrategyConfig(gamma=0.1, sigma=0.5, k=1.5, order_qty=5, fixed_spread=0.2)
    ...
    for step in range(steps):
        ...
        mid_price = (best_bid + best_ask) / 2.0 if best_bid and best_ask else gen_config.start_price
...
if __name__ == "__main__":
    ...
    # Inventory Plot
    ax2.plot(df_naive['time'], df_naive['inventory'], label='Naive Inventory', color='red', alpha=0.6)
    ax2.plot(df_as['time'], df_as['inventory'], label='A-S Inventory', color='blue', alpha=0.6)
