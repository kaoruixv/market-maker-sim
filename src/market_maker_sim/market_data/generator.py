...
class OrderFlowGenerator:
    ...
    def next_event(self) -> Tuple[float, Optional[Order], Optional[int]]:
        ...
        if rand_val < self.config.mu:
            ...
            cancel_id = self.rng.choice(list(self.book.order_map.keys())) if self.book.order_map else -1
    ...
    def _gen_order(self, order_type: OrderType, force_price: bool = False) -> Order:
        ...
        if order_type == OrderType.MARKET:
            ...
            mid = self.config.start_price if not best_bid or not best_ask or force_price else (best_bid + best_ask) / 2.0
            tick_offset = self.rng.randint(1, 5) * self.config.tick_size
            price = round(mid - tick_offset if side == Side.BID else mid + tick_offset, 2)
