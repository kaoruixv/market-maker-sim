...
class ASMarketMaker(BaseMarketMaker):
    def generate_quotes(self, mid_price: float) -> list[Order]:
        ...
        reservation_price = mid_price - (
            self.inventory * self.config.gamma * (self.config.sigma**2)
        )
        spread = (self.config.gamma * (self.config.sigma**2)) + (
            (2.0 / self.config.gamma)
            * math.log(1.0 + (self.config.gamma / self.config.k))
        )
        ...
        if self.inventory + self.config.order_qty <= self.config.position_limit:
            orders.append(
                Order(
                    ...
                    Side.BID,
                    self.config.order_qty,
                    round(reservation_price - half_spread, 2),
                )
            )
            self.order_id_counter += 1
