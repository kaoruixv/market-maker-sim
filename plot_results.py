...
def plot_simulation_data():
    ...
    ax1.plot(df['Step'], df['MidPrice'], label='Mid Price', color='black', linewidth=1.5)
    ax1.plot(df['Step'], df['Quote_Ask'], label='Agent Ask', color='red', linestyle='--', alpha=0.7)
    ax1.plot(df['Step'], df['Quote_Bid'], label='Agent Bid', color='green', linestyle='--', alpha=0.7)
    ...
    try:
        ...
        print("Interactive window not supported in this terminal, but the PNG is saved!")
