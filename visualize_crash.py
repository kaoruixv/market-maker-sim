...
def run_visual_crash():
    ...
    for step in range(1, 101):
        ...
        prob_bid = min(1.0, math.exp(-agent.k * delta_bid)) if bid != -float('inf') else 0.0
        prob_ask = min(1.0, math.exp(-agent.k * delta_ask)) if ask != float('inf') else 0.0
    ...
    ax2.axhline(-agent.max_inventory, color='red', linestyle=':', label="Max Short Limit")
